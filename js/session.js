...
const Session = {
  ...
  view(){
    ...
    const startBtn = el('button.btn' + (running ? '.danger' : '.go'), {
      onclick: () => {
        if(running){
          ...
          ses.startedAt = Date.now(); Session._lastTick = Date.now(); Store.save(); App.render();
    ...
    page.appendChild(el('div.study-head', {}, [
      ...
      el('button.btn.sm', { text: 'PiP', title: 'Pop out the live panel', onclick: () => Session.openPiP() }),
      startBtn
    ]));
  ...
  renderStats(ses){
    ...
    [
      ...
      rail.appendChild(el('div.stat' + (live ? '.live' : ''), {}, [
        ...
        el('div.stat-v', { text: v, style: k === 'Now' ? { fontSize: '14px', fontFamily: "'Space Grotesk',sans-serif" } : {} })
      ]));
    });
  },
  ...
  async openPiP(){
    const ses = this.active();
    if(!ses){ toast('Open a session first.', true); return; }
    if(!('documentPictureInPicture' in window)){
      toast('Your browser has no picture-in-picture for pages. Chrome or Edge 116+ can do it.', true);
      return;
    }
    try{
      ...
      style.textContent = `
        ...
        .now{margin:10px 0 12px}
        .t{font-size:14px;font-weight:600;margin-top:2px}
        ul{margin:8px 0 0;padding-left:16px;font-size:11px;color:#8b97a8}
      `;
      ...
      this.updatePiP(ses);
    }catch(e){ toast('Could not open the picture-in-picture window.', true); }
  },
