...
const Dashboard = {
  ...
  dayCard(d, label){
    ...
    return el('div.card.daycard', {}, [
      ...
      body,
      el('div.daycard-foot', {}, [
        el('button.btn.primary', { text: label === 'Today' ? 'Start study' : 'Plan a session', onclick: () => {
          if(label === 'Today') Session.startToday();
          else { const s = Session.create(dk, 'Study session', false); s.hours = Store.state.settings.defaultHours; Store.save(); App.go('session'); }
        }})
      ])
    ]);
  }
