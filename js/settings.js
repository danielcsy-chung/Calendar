...
const Settings = {
  ...
  /* ---------- 1 & 2. import ---------- */
  importCard(){
    const ta = el('textarea', { placeholder: 'Paste CSV lines here, or choose a file below…', style: { minHeight: '150px', fontFamily: "'JetBrains Mono',monospace", fontSize: '12px' } });
  ...
  subjectsCard(){
    ...
    const draw = () => {
      ...
      Store.state.subjects.forEach((s, i) => {
        ...
        const col = el('input', { type: 'color', value: s.color });
        col.addEventListener('input', () => { s.color = col.value; Store.save(); });
    ...
    return el('div.card.set-card', {}, [
      ...
      el('p.hint', { text: 'The colour follows the subject everywhere: blocks, chips, charts.' }),
      list,
